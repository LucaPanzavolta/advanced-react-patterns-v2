...
class Toggle extends React.Component {
  ...
  getState = () => (this.isOnControlled() ? {on: this.props.on} : this.state); 
  ...
  toggle = () => {
    ...
    console.log({getState: !this.getState().on})
    this.isOnControlled() ? this.props.onToggle(!this.getState().on) : this.setState(({on}) => ({on: !on}), () => this.props.onToggle(this.getState().on));
  }
  render() {
...
class Usage extends React.Component {
  state = {bothOn: false}
  handleToggle = on => {
    console.log("function called with ", on)
